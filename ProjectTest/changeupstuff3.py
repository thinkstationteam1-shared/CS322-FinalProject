...
class LocalLLM:
    ...
    def __init__(self, model_id: str, token: str):
        ...
        log.info("[LocalLLM.__init__] Tokenizer loaded. Loading model weights (this may take a while)...")
        ...
        log.info(f"[LocalLLM.__init__] Model loaded. Device map: {self.model.hf_device_map}")
    ...
    def generate(self, system: str, user: str, max_tokens: int = 256) -> Optional[str]:
        ...
        log.debug(f"[LocalLLM.generate] Prompt encoded: {input_len} tokens. Starting generation...")
        ...
        if gen_elapsed > 60:
            log.warning(f"[LocalLLM.generate] SLOW GENERATION: took {gen_elapsed:.1f}s — possible stall")
