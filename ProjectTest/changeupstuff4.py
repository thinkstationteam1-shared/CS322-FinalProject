...
# --- GPU / device probe ---
# torch.cuda.device_count() uses a different NVML path than bitsandbytes and
# works even when bitsandbytes' NVML init fails, so we use torch directly.
_cuda_ok = torch.cuda.is_available()
log.info(f"[init] torch.cuda.is_available() = {_cuda_ok}")
log.info(f"[init] GPUs visible to torch: {torch.cuda.device_count()}")
for _i in range(torch.cuda.device_count()):
    log.info(f"[init] Device {_i}: {torch.cuda.get_device_name(_i)}")
DEVICE = "cuda" if _cuda_ok else "cpu"
log.info(f"[init] Will load model to device='{DEVICE}'")
...
class LocalLLM:
    """
    Loads Llama 3.1 in float16 without bitsandbytes quantization.

    Why no 4-bit quant: bitsandbytes==0.45.0 in this environment was compiled
    without GPU support (NVML init fails), so BitsAndBytesConfig raises a
    RuntimeError even though torch itself can see the CUDA device.  Loading in
    float16 with device_map="auto" lets transformers/accelerate shard the model
    across available GPUs (or fall back to CPU) without touching bitsandbytes.

    Memory note: Llama-3.1-8B-Instruct in fp16 needs ~16 GB of GPU VRAM.
    If you only have one 24 GB card it fits; if VRAM is tight pass
    --torch_dtype float32 is NOT recommended — use --no_half instead (see arg).
    """

    def __init__(self, model_id: str, token: str, use_half: bool = True):
        log.info(f"[LocalLLM.__init__] Loading model: {model_id}")
        log.info(f"[LocalLLM.__init__] use_half={use_half}, target device='{DEVICE}'")
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            model_id,
            token=token,
            # tokenizers 0.20.3 — no extra kwargs needed
        )
        log.info("[LocalLLM.__init__] Tokenizer loaded. Loading model weights (may take several minutes)...")

        # device_map="auto" uses accelerate to shard across GPUs / CPU.
        # low_cpu_mem_usage=True is the accelerate-backed path that avoids
        # materialising the full model in CPU RAM before moving to GPU.
        ...
        # hf_device_map only exists when device_map="auto" was used
        device_map_info = getattr(self.model, "hf_device_map", "N/A (single device)")
        log.info(f"[LocalLLM.__init__] Model loaded. dtype={dtype} | device_map={device_map_info}")

    @property
    def _first_device(self) -> torch.device:
        """Return the device of the first model parameter (works with multi-GPU sharding)."""
        return next(self.model.parameters()).device

    def generate(self, system: str, user: str, max_tokens: int = 256) -> Optional[str]:
        log.debug(f"[LocalLLM.generate] Encoding prompt (user msg length: {len(user)} chars)")
        messages = [
            {"role": "system", "content": system},
            {"role": "user",   "content": user},
        ]

        # apply_chat_template returns a plain tensor dict in transformers 4.46
        ...
        # Move input tensors to whichever device holds the first layer.
        # With device_map="auto" the embeddings are always on the first device.
        first_dev = self._first_device
        encodings = {k: v.to(first_dev) for k, v in encodings.items()}

        input_len = encodings["input_ids"].shape[-1]
        log.debug(f"[LocalLLM.generate] Prompt encoded: {input_len} tokens on {first_dev}. Generating...")
        ...
        log.debug(
            f"[LocalLLM.generate] Done. Generated {len(response_ids)} tokens "
            f"in {gen_elapsed:.1f}s. Response length: {len(decoded)} chars"
        )
        if gen_elapsed > 60:
            log.warning(f"[LocalLLM.generate] SLOW GENERATION: {gen_elapsed:.1f}s — possible stall or CPU fallback")
