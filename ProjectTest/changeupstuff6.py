...
#_GPU_CAP_GIB = 10
_GPU_PRIMARY_CAP_GIB = 18
_GPU_SPILLOVER_CAP_GIB = 3
_gpu_caps = {1:_GPU_PRIMARY_CAP_GIB, 0:_GPU_SPILLOVER_CAP_GIB}
_gpu_vram: dict = {}
for _i in range(_num_gpus):
    _props = torch.cuda.get_device_properties(_i)
    _total_gib = _props.total_memory / (1024 ** 3)
    _cap = _gpu_caps.get(_i, _GPU_SPILLOVER_CAP_GIB)
    _usable_gib = min(_cap, int(_total_gib))
...
class LocalLLM:
    ...
    def __init__(self, model_id: str, token: str, use_half: bool = True):
        ...
        if _num_gpus >= 2:
            max_memory = {0: _gpu_vram[0], 1:_gpu_vram[1], "cpu": "0GiB"}
